# -*- coding: utf-8 -*-
"""v27：在二进制 AndroidManifest.xml 里就地改版本信息。
...
SRC = os.path.join(BASE, "manifest_a5_v26.bin")
DST = os.path.join(BASE, "manifest_a5_v27.bin")
...
# ---- 1) versionName: V26 -> V27 ----
pos = find_u16(d, "V26")
if pos < 0:
    raise SystemExit("找不到 versionName 字符串 V26")
old = bytes(d[pos:pos + 10])          # len(2) + 3*2 + NUL(2)
new = struct.pack("<H", 3) + "V27".encode("utf-16-le") + b"\x00\x00"
assert len(old) == len(new) == 10, (len(old), len(new))
d[pos:pos + 10] = new
print("versionName 替换:", old, "->", bytes(new))

# ---- 2) versionCode: 690 -> 691 ----
sys.path.insert(0, BASE)
import axml_info

hit = axml_info.find_attr_value_offset(bytes(d), "manifest", "versionCode", expect_int=690)
if not hit:
    raise SystemExit("找不到 manifest 的 versionCode 属性（或值不是 690）")
_p, voff = hit
struct.pack_into("<i", d, voff, 691)
print("versionCode 690 -> 691 @%d，核对 = %d" % (voff, struct.unpack_from("<i", d, voff)[0]))
...
assert i1.get("versionCode") == 691, "versionCode 未改成功"
assert i1.get("versionName") == "V27", "versionName 未改成功"
...
print("复核通过：SDK/包名未变，版本已升到 V27(691)")
...
print("字符串差异:", diff)
if len(diff) != 1 or diff[0][1] != "V26" or diff[0][2] != "V27":
    raise SystemExit("差异不符合预期，中止")
print("复核通过：仅 versionName V26 -> V27，其余 %d 条字符串不变" % (len(a) - 1))
