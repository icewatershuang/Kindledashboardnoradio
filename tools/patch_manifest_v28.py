# -*- coding: utf-8 -*-
"""v28：在二进制 AndroidManifest.xml 里就地改版本信息。
...
SRC = os.path.join(BASE, "manifest_a5_v27.bin")
DST = os.path.join(BASE, "manifest_a5_v28.bin")
...
# ---- 1) versionName: V27 -> V28 ----
pos = find_u16(d, "V27")
if pos < 0:
    raise SystemExit("找不到 versionName 字符串 V27")
old = bytes(d[pos:pos + 10])          # len(2) + 3*2 + NUL(2)
new = struct.pack("<H", 3) + "V28".encode("utf-16-le") + b"\x00\x00"
assert len(old) == len(new) == 10, (len(old), len(new))
d[pos:pos + 10] = new
print("versionName 替换:", old, "->", bytes(new))

# ---- 2) versionCode: 691 -> 692 ----
sys.path.insert(0, BASE)
import axml_info

hit = axml_info.find_attr_value_offset(bytes(d), "manifest", "versionCode", expect_int=691)
if not hit:
    raise SystemExit("找不到 manifest 的 versionCode 属性（或值不是 691）")
_p, voff = hit
struct.pack_into("<i", d, voff, 692)
print("versionCode 691 -> 692 @%d，核对 = %d" % (voff, struct.unpack_from("<i", d, voff)[0]))
...
assert i1.get("versionCode") == 692, "versionCode 未改成功"
assert i1.get("versionName") == "V28", "versionName 未改成功"
...
print("复核通过：SDK/包名未变，版本已升到 V28(692)")
...
print("字符串差异:", diff)
if len(diff) != 1 or diff[0][1] != "V27" or diff[0][2] != "V28":
    raise SystemExit("差异不符合预期，中止")
print("复核通过：仅 versionName V27 -> V28，其余 %d 条字符串不变" % (len(a) - 1))
